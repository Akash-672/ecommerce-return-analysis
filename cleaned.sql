...
-- The above both query is to test wheather we can convert the date into standard date format

SET SQL_SAFE_UPDATES = 0; -- Safe mode to change anything was ON by default I turned off using this query.
...
/* Not as described has the highest revenur loss
   followed by the defective and missing/wrong item and soo run the above query to check the numbers
*/
...
/* electronics, homes, fashion, sports an so on has the highest revenue loss from returns
	run the above query to see the numbers
*/
...
/* 1-10% discount have the highest return_rate
   followed by 11-20%, 0%, 21-30% etc..alter
*/
...
-- south return region have the highest revenue lost followed by east,west etc.. run the above query to see the numbers
